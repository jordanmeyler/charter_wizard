...
- **Interact** (**E**, or the **Pray** / verb button when you stand in range) uses a nearby `WorldInteract`. Drop that component on an empty GameObject and dress the statue with tiles — it does not draw its own sprite. Prayer shows a written spell with each mark labelled elemental, catalyst, or special. **Cast** aims that working (it counts in Recent and can be kept / named); **Continue** leaves it.
