...
- The **top-left** panel is a running look: the room, what you are looking at (`You see …`), your statuses, and a **scrollable log** of the last few dozen lines of play (newest at the bottom). **–** collapses it to a strip that still names the look. Statuses live here — burning, poison, wards, forms, and the rest — and only while they hold. When nothing is on you it says **Everything's ok**. A clock that reaches 0 leaves. Mouse-over or the current target fills the look line. Workings are named by the runes you wrote, or by a name you saved for that same composition (Spark is not Fire · Air). Pack, interact, and yield stay on **I** / **E** / **K**.
...
- The **bottom bar** holds the stored Charter spell and the main controls as buttons with their keys: **Charter** (Space), **Cast** (F), **Free** (X), **Store** (R), **Use** (E, when something is in reach), **Yield** (K), then **Pause**, **Play** / **Develop**, **Pack**, and **Book**. Click the slot or press **F** / **Enter** in the world to aim a held working.
...
- **Grimoire** (top-right tab, or the bottom-bar **Book** / **G**) is the player's book. It fills the screen above the bar and only scrolls when a page is taller than the window. **Search** filters by name, recipe, or meaning. The **rune chips** keep only pages that involve that mark (Spark counts as Fire). Play shows kept workings; **Develop** (F1) adds **Runes**, **Spells**, and **World** tabs. Click a join or a spell to string it if those marks are in the weave; otherwise a notice names the missing marks. A kept Free working strings the same marks again and Free chooses once more — it may not land the same way. The world still speaks the runes you wrote, or the name you gave that writing. **+** on a kept page renames it. Esc clears the search, then the filter, then closes the book.
