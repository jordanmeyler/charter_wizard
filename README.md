...
- **Interact** (**E**, or the **Pray** / verb button when you stand in range) uses a nearby `WorldInteract` or `WorldSpeech`. Drop Interact on an empty GameObject and dress the statue with tiles — it does not draw its own sprite. Prayer shows a written spell with each mark labelled elemental, catalyst, or special. **Cast** aims that working (it counts in Recent and can be kept / named); **Continue** leaves it. Drop **Speech** on a Gate (or a volume beside it) for a one-shot approach window. Drop **Sign** or **Talk** for Read / Talk — same E button, a text window instead of a prayer. Extra pages are a first step toward conversation trees. Esc closes the window; E / Space / Enter / Continue advances.
