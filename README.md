...
- In the Charter: **click weave cells** to string them (up to 8). Play shows only the grid — no wall of root marks. The weave is **what the camera can see** — if it is on screen, it is valid. Hover the clipped belt to still it and see where a mark is from. Generation puts **several of each available rune** in the grid so a lone tile stays clickable; extra copies follow how often that material appears, with uncommon marks taking a larger share (a floor of stone still speaks more Earth than a puddle speaks Water). Develop still names the eleven and the elemental joins on a wall; a join that is not in view stays grey. **Air is ambient** wherever a room still holds breath. **You** are always mind · body · soul. Two runes birth a join or wait; a finished spell is a sentence. Then **Charter Cast**, **Store**, or **Free Cast**.
...
- **Recent casts** (top-right): the last twenty-five attempts. The **play** mark strings it again; **+** opens a naming modal (the game pauses and keyboard controls lock) that shows the rune combo and keeps that exact writing in the Grimoire. The saved name is used only for the same composition. Esc cancels.
