...
- The **top-left** panel is a running look: the room, what you are looking at (`You see …`), your statuses, and the last line of play. **–** collapses it to a strip that still names the look. Statuses live here — burning, poison, wards, forms, and the rest — and only while they hold. When nothing is on you it says **Everything's ok**. A clock that reaches 0 leaves. Mouse-over or the current target fills the look line. Workings are named by the runes you wrote, or by a name you saved for that same composition (Spark is not Fire · Air). Pack, interact, and yield stay on **I** / **E** / **K**.
