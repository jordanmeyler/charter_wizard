...
- In the Charter: **click wall runes** or weave cells to string them (up to 8). The weave is **what the camera can see** — if it is on screen, it is valid. Hover a mark to see where it is from. Generation puts **at least one of each available rune** in the grid; extra copies follow how often that material appears (a floor of stone speaks more Earth than a puddle speaks Water). Play also offers kept marks and any wrought join the room is speaking (Spark, Ice, Plant…). Develop names the eleven and the elemental joins; a join that is not in view stays grey. **Air is ambient** wherever a room still holds breath. **You** are always mind · body · soul. Two runes birth a join or wait; a finished spell is a sentence. Then **Charter Cast**, **Store**, or **Free Cast**.
...
- **Grimoire** (top-right tab, or the bottom-bar book / **Esc** / **G**) is the player's book in Play: only workings you **Keep** from Recent. Click one to string it — the eleven roots are always ready. **Develop** (F1) shows the full written catalog and every wrought join. The world still speaks the runes you wrote, or the name you gave that writing.
