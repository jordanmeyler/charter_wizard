...
- **Interact** (**E**, or the **Pray** / verb button when you stand in range) uses a nearby `WorldInteract` or `WorldSpeech`. Drop Interact on an empty GameObject and dress the statue with tiles — it does not draw its own sprite. Set the **recipe runes** (and an optional second writing). Prayer shows that recipe, not a locked catalog name; if the working has another writing it is shown too. **Cast** aims the shown chain (it counts in Recent and can be kept / named); **Continue** leaves it. Drop **Elemental Altar** to show how a join is made — Fire and Air, an equals, then Spark. Drop **Speech** on a Gate (or a volume beside it) for a one-shot approach window. Drop **Sign** or **Talk** for Read / Talk — same E button, a text window instead of a prayer. Extra pages are a first step toward conversation trees. Esc closes the window; E / Space / Enter / Continue advances.
