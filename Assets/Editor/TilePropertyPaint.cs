...
namespace RuneMagic
{
    ...
    public sealed class TilePropertyPaint : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox(
                "Select the layer first. A cell is floor only if you stamp Kind = Floor or paint a Floor brush. Looks on any layer — including extra Floor / Tiles children — are not walkable until stamped. Environment Details is its own stamp — check Blocks, or stamp Kind = Wall, to give a cluster collision. Wall stamps keep collision; a Floor stamp never punches a hole in masonry. Stamps sit over the tile you painted. Plant growth does not start itself. Cover is its own Tilemap layer: paint Cover-Fire, Cover-Vine, ice, miasma there so a substance lands on a cell and reacts with that cell's stamp. Floor-Fire and Wall-Fire are rest matter on Tiles / Walls — they are not covers. A cover on that same cell can still light. They do not light a neighboring plant floor or wall. Cover-Fire burns who stands on it; it does not jump to the plant beside a torch. Floors, walls, oil, and details stay dark until a cover lands on them or a spell starts hunger. Floor and wall stamps keep the tileset sprite they sit on; they do not swap in Floor-Stone / Floor-Plant / Floor-Fire pack art, and they do not draw a second graphic on top. Cover-* / Aura-* and spell leftovers may sit on that same tile. A watered land plant may grow onto one neighboring water floor or water covering. Plant cover on water stays put, like ice, unless Forest drinks the visible pool. Hunger spent on timber or plant swaps that stamp and floor tile to leftover dirt (look and Earth). It does not draw ash over the tile you placed. Fire cover stays. Masonry stays. Cover-* and spell leftovers may still sit on that leftover. Cover is the overlay: look, work, and the same catalog mark as an inscription. Ice is Water · Earth. Fire cover burns who stands on it — the weave speaks Fire, and you can click the mark. It does not kindle a hall. A later fireball, spreading burn, or oil a spell left will still find that cover. Ice cover melts when hunger crosses it. Oil or metal stamped on the Cover layer is fuel or a path for the spark — the stamp does not start the reaction. Vine is Plant · Mercury. Ash is Fire · Plant. Miasma is Cloud · Acid. Fog is Cloud. A Water material stamp keeps the tile you painted. Cover-Water may generate the Water mark. Write onto Cover layer (or select Cover) so a stamp does not change Kind. Play shows the generated mark; click it to draw that rune. Material = Miasma on Cover is the same as Cover = Miasma. Material = Fire on Cover is not Cover-Fire — set Cover = Fire to paint live hunger. Miasma and fog are see-through unless you stamp Opacity. Blank Tiles cells are pits at Play.",
                MessageType.None);
        }
        ...
        static Sprite SpriteOf(Tilemap map, Vector3Int cell, TileBase tile)
        {
            ...
            if (tile is WorldPaintTile quality && (quality.IsQualityStamp || quality.IsOverlayBrush))
            {
                return null;
            }
