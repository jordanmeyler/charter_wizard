...
There is **no generic hit-point bar**. A spell kills, restrains, or does neither (traverse, heal, hide, lift, summon, transform). Kill is binary: `power × affinity > defense`. Affinity is **0–5** (0 immune, 5 five-times). Defense is **1–8** (7–8 are high beings; a stone golem is **4**). Power is **0–10**. Power 0 never kills. Power 1 cannot beat defense 7–8 even at affinity 5. Witchfire is always read as 1 and hits for **7**. Glacier sent is **6**. Unmake is the one elementless chaos sentence at **10**. Fire, lava, witchfire, and plasma are **different columns**. Charm, rage, lull, terror, confuse, command, and stun are **different columns**. Wind does not kill — it **pushes**; air ward / gale-form refuse the shove. **Burning** still runs a contact meter. **Poison** is slower (fourteen breaths) and **does not lift** until Light · Salt · Water · Mercury (Cleanse). Rooted prey stay in hunger so a vine plus a weak fire can finish what a fireball could not. Bodies can **strike back** — a slam or a flying shot will send the adept to the spawn crystal — and spells leave **visible statuses** that different natures take differently.
