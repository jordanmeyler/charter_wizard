...
**Targeting is written with the spell.** Single-target work finds the nearest lock at the click. Area work (Rain, Live-floor, Thunderclap, Sprout…) offers the key to every lock in the radius and paints the tiles. Self work (Hop, Float, Flight, wards, forms, Veil) stays on the caster. Grow form widens a single sentence into an area around the feet. Runtime table: `SpellVerb`.
