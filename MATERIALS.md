...
| **Fire** | Fire | Fire | Rest stamp. Weak source, not fuel. Speaks Fire. The room is at rest: a cover on that same cell can still light. It does not light a floor or wall beside it until a spell starts hunger. Cover-Fire is the live Cover-layer substance: standing on it burns, and it reacts with the stamp on that cell. |
...
A **kindled hall** is the **Aura-Fire** brush (or a Flame Hall plaque): painted walk that stays lit until yield is thrown. It is not a material — it is a source. Live Aura-Fire, a geyser, or lit Floor-Fire counts as Hunger **10**. Floor-Fire / Wall-Fire stay at rest until a spell or covering on that same cell starts work — plant on that walk lights the vine, not the masonry. They do not light the plant floor beside a torch. Vine is a wick a spell can run, and a burning plant covering lights adjacent wood and oil. Stamp **Cover-Vine** on the Cover layer (or write Vine / Sprout). Floor-Plant and Floor-Grove catch when a cover or spell is on that cell; they do not run a field from a neighboring wall. **Ember** is coals: it provides fire, accepts a flame from another source, and lets hunger walk across. The tile underneath stays embered even after everything around it burns. Neutral stone / dirt never catch from a neighbor.
